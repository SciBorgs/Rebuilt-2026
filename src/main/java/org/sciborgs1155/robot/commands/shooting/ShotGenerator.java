...
public final class ShotGenerator {
  ...
  public static double[] fuelExitPosition(double hoodAngle, Pose3d robotPose) {

    double fuelX = Centimeters.convertFrom(HOOD_RADIUS.magnitude(), Inches) * Math.sin(hoodAngle) + shooterPose(robotPose)[X];
    double fuelZ = Centimeters.convertFrom(HOOD_RADIUS.magnitude(), Inches) * Math.cos(hoodAngle) + shooterPose(robotPose)[Z];

    double[] fuelExitPose = {fuelX, shooterPose(robotPose)[Y], fuelZ};
    return fuelExitPose;
  }
