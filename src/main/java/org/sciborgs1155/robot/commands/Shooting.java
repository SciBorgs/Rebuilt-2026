...
___________________________________________________________________________________________________________________
  ...
  */
  ...
  private static final Translation3d hub =
      new Translation3d(
          0, 0,
          0); // put this in constants please lwaeiojfaopiwejfpowj and actually find the position on
  ...
  /**
   * TODO Runs the shooter before feeding it the note.
   *
  ...
  /**
   * TODO Runs shooter to desired velocity, runs feeder once it reaches its velocity and
   * shootCondition is true.
   *
  ...
  public Command shootWhileDriving(
    ...
    return shoot(
            ...
            () ->
                atYaw(
                    yawFromNoteVelocity(
                        calculateFuelVelocity()))) // atYaw --> this will be a turret function w/ a
        // tolerance. might need to have separate
        // functions for when the robot moves
        // horizontally relative to hub
        .deadlineFor(
            drive.drive(
                vx.scale(
  ...
  public boolean atYaw(Rotation2d yaw) { // turret command instead of robot positioning
    ...
    double tolerance =
        DriveConstants.Rotation.TOLERANCE.in(Radians)
            * (1 - yaw.getSin()); // figure out the math over here and replace with a smaller prive
    // funciton
  ...
  public Vector<N3> calculateFuelVelocity(
      Time
          predictionTime) { // this code structure seems to account for the extra yaw / pitch that I
    // was looking for before
    return calculateFuelVelocity(
        constantVelocityPredictedPose(
            drive.pose(), drive.fieldRelativeChassisSpeeds(), predictionTime));
  }
   ...
   */
  public Vector<N3> calculateFuelVelocity(
      Pose2d robotPose) { // is it necessary at all to have the orientation of the fuel on hand? I
    // don't understand this part as much
    ...
    /*  */
    double shotVelo =
        calculateStationaryVelocity(
            difference
                .getNorm()); // can we not just get the norm in all cases ? Is this ever useful for
    // any type of orientation thingy
    ...
    Vector<N3> fuelVelocity =
        new Translation3d(1, 0, 0).rotateBy(fuelOrientation).toVector().unit().times(shotVelo);

    return fuelVelocity.minus(robotVelocity);
  }

  public static Pose2d constantVelocityPredictedPose(
      Pose2d robotPose,
      ChassisSpeeds speeds,
      Time predictionTime) { // TODO finish this sometime soon
    ...
    Vector<N3> predicted =
        position.plus(
            velocity.times(
                predictionTime.in(Seconds))); /* how is this prediciton time calculated? */
  ...
  public static double pitchFromNoteVelocity(Vector<N3> velocity) {
    ...
    /* (z / x) /  y  */
    // this is the formula for angle between rho and the base vector
  }
  ...
  public static double rotationalVelocityFromFuelVelocity(Vector<N3> velocity) {
    return velocity.norm()
        / fuel.RADIUS.in(Meters)
        * siggysConstant
            .get(); // make some constants and valuess to reflect, see if we want to still honor
    // siggysConstant?
  }
  ...
  public static Translation2d translationToHub(Translation2d robotTranslation) {
    return hub()
        .toTranslation2d()
        .minus(
            robotTranslation); // TODO speaker --> make into hub --> ankit did set those things up
    // which is very commendable and i thank him for that
  }

  public static double calculateStationaryVelocity(
      double distance) { // TODO HOOD Angle is flexible as well
